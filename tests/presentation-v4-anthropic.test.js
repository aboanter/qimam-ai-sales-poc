...
const {callAnthropicArtDirector,firstText,truncationError,API_URL}=require('../presentation-v4-anthropic');
...
(async()=>{
  ...
  const fakeFetch=async(url,options)=>{
    ...
    return new Response(JSON.stringify({
      ...
      content:[{type:'text',text:'{"title":"X","components":[{"type":"kpi","title":"T","dataset":"sales","field":"amount_total:sum"}]}'}]
  ...
  assert.ok(/under 900 tokens/.test(captured.body.system));
  ...
  assert.strictEqual(truncationError({stop_reason:'end_turn'},1200),null);
  assert.match(truncationError({stop_reason:'max_tokens',usage:{output_tokens:1200}},1200).message,/truncated at max_tokens=1200 after 1200 output tokens/);

  const truncatedFetch=async()=>new Response(JSON.stringify({
    stop_reason:'max_tokens',usage:{input_tokens:150,output_tokens:1200},content:[{type:'text',text:'{"title":"cut"'}]
  }),{status:200,headers:{'content-type':'application/json'}});
  await assert.rejects(()=>callAnthropicArtDirector({prompt:'x',apiKey:'k',fetchImpl:truncatedFetch,maxTokens:1200}),/truncated at max_tokens=1200/);
