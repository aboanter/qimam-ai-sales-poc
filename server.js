...
app.post('/api/plan',async(req,res)=>{const{question,previousPlan}=req.body||{};if(!question||typeof question!=='string'||question.length>500)return res.status(400).json({error:'A question string (max 500 chars) is required.'});try{const userText=(previousPlan?`Previous plan for conversational context:\n${JSON.stringify(previousPlan)}\n\n`:'')+`User question:\n${question}`;const{text,model}=await callAnthropic(PLAN_SYSTEM_PROMPT,userText);const plan=parseJsonLoose(text);validatePlan(plan);return res.json({plan,rawText:text,model});}catch(err){return res.status(err.status||502).json({error:err.message,detail:err.detail});}});
