// Background Jobs Client V1.2 — resumable jobs with manual recovery and a guaranteed way to start a new question.
(function(){
  ...
  function makeCard(question){
    ...
    const abandon=el('button','qbgjob-new','＋ بدء سؤال جديد');abandon.type='button';
    abandon.addEventListener('click',()=>abandonCurrent(true));
    actions.append(refresh,abandon);
  ...
  function setChecking(card,on){if(!card)return;const b=card.querySelector('.qbgjob-refresh');if(b){b.disabled=!!on;b.textContent=on?'جارٍ التحقق…':'↻ تحديث الحالة الآن'}}
  ...
  async function getJob(id){const r=await fetch('/api/jobs/'+encodeURIComponent(id)+'?t='+Date.now(),{cache:'no-store',headers:{'cache-control':'no-cache'}});const d=await r.json().catch(()=>({}));if(r.status===404){const e=new Error('الطلب السابق لم يعد موجوداً على السيرفر.');e.code='NOT_FOUND';throw e}if(!r.ok)throw new Error(d.error||'تعذر استرجاع حالة الطلب');return d.job}
  ...
  async function poll(manual=false){
    ...
    try{
      ...
      if(e.code==='NOT_FOUND'){
        clearTimeout(timer);timer=null;showError(currentCard,e.message);setStored(null);currentId=null;return;
      }
  ...
  function install(){
    ...
    document.addEventListener('click',function(ev){
      ...
      if(currentId){
        const ok=window.confirm('يوجد طلب سابق ما زال قيد المتابعة. هل تريد إيقاف متابعته وبدء السؤال الجديد؟');
        if(!ok)return;
        abandonCurrent(true).then(()=>submit(question));
        return;
      }
      submit(question);
    },true);
...
@media(max-width:620px){.qbgjob-status{align-items:flex-start;padding:15px}.qbgjob-line{gap:8px}.qbgjob-actions{grid-template-columns:1fr}.qbgjob-refresh,.qbgjob-new{width:100%;padding:11px}}
`;
