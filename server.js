...
- sale.order: id,name,date_order,state,partner_id,user_id,company_id,currency_id,amount_untaxed,amount_tax,amount_total,invoice_status,order_line. Confirmed sales normally use state='sale'. Date grouping supports date_order:month, date_order:quarter, date_order:year.
...
- account.move: id,name,invoice_date,date,state,move_type,partner_id,company_id,amount_untaxed,amount_tax,amount_total,amount_residual,payment_state.
...
- Respect dates exactly when specified. Current date is 2026-09-05.
- Never request write tools.
...
async function mcpPost(body,sessionId=''){if(!ODOO_MCP_URL||!ODOO_MCP_KEY)throw Object.assign(new Error('Odoo MCP is not configured on the server.'),{status:503});const h={'Authorization':`Bearer ${ODOO_MCP_KEY}`,'Content-Type':'application/json','Accept':'application/json','MCP-Protocol-Version':MCP_PROTOCOL};if(sessionId)h['Mcp-Session-Id']=sessionId;const r=await fetch(ODOO_MCP_URL,{method:'POST',headers:h,body:JSON.stringify(body)});const text=await r.text();if(!r.ok)throw Object.assign(new Error(`Odoo MCP returned HTTP ${r.status}`),{status:502,detail:text.slice(0,800)});let data=null;if(text.trim()){try{data=JSON.parse(text)}catch{data={raw:text}}}return{data,sessionId:r.headers.get('mcp-session-id')||sessionId};}
async function createMcpSession(){const init=await mcpPost({jsonrpc:'2.0',id:1,method:'initialize',params:{}});if(!init.sessionId)throw Object.assign(new Error('Odoo MCP did not return a session id.'),{status:502});await mcpPost({jsonrpc:'2.0',method:'notifications/initialized'},init.sessionId);return init.sessionId;}
...
function validatePlan(plan){if(!plan||!Array.isArray(plan.operations)||!plan.operations.length||plan.operations.length>6)throw new Error('Invalid or empty query plan.');for(const op of plan.operations){if(!op.name||!ALLOWED_TOOLS.has(op.tool))throw new Error(`Unsafe/unsupported MCP tool: ${op.tool}`);if(!op.arguments||!ALLOWED_MODELS.has(op.arguments.model))throw new Error(`Unsafe/unsupported Odoo model: ${op.arguments&&op.arguments.model}`);if(op.arguments.limit&&op.arguments.limit>100)op.arguments.limit=100;if(op.tool==='read_group'&&(!Array.isArray(op.arguments.groupby)||!op.arguments.groupby.length))op.arguments.groupby=['company_id'];}}
function unpackMcpToolResult(data){if(!data)return null;if(data.error)return{error:data.error};const blocks=data.result&&data.result.content;if(!Array.isArray(blocks))return data.result||data;const text=blocks.filter(x=>x.type==='text').map(x=>x.text||'').join('\n').trim();if(!text)return data.result;try{return JSON.parse(text)}catch{return text}}
async function executeOdooPlan(plan){const sessionId=await createMcpSession();const results={};let id=10;for(const op of plan.operations){const response=await mcpPost({jsonrpc:'2.0',id:id++,method:'tools/call',params:{name:op.tool,arguments:op.arguments}},sessionId);results[op.name]=unpackMcpToolResult(response.data);}return results;}
app.post('/api/plan',async(req,res)=>{const{question,previousPlan}=req.body||{};if(!question||typeof question!=='string'||question.length>MAX_QUESTION_CHARS)return res.status(400).json({error:`A question string (max ${MAX_QUESTION_CHARS} chars) is required.`});try{const userText=(previousPlan?`Previous plan for conversational context:\n${JSON.stringify(previousPlan)}\n\n`:'')+`User question:\n${question}`;const{text,model}=await callAnthropic(PLAN_SYSTEM_PROMPT,userText);const plan=parseJsonLoose(text);validatePlan(plan);return res.json({plan,rawText:text,model});}catch(err){return res.status(err.status||502).json({error:err.message,detail:err.detail});}});
app.post('/api/execute',async(req,res)=>{try{const{plan}=req.body||{};validatePlan(plan);const results=await executeOdooPlan(plan);return res.json({results,source:'live_odoo_mcp'});}catch(err){return res.status(err.status||502).json({error:err.message,detail:err.detail});}});
const VALID_COMPONENT_TYPES=['kpi','table','bar_chart','line_chart','area_chart','pie_chart','scatter_chart','insight'];
function validateSchema(s){return !!(s&&typeof s==='object'&&typeof s.title==='string'&&Array.isArray(s.components)&&s.components.every(c=>c&&VALID_COMPONENT_TYPES.includes(c.type)));}
app.post('/api/present',async(req,res)=>{const{question,plan,results}=req.body||{};if(!question||!plan||!results)return res.status(400).json({error:'question, plan and results are required'});try{const prompt=`Original question:\n${question}\n\nExecuted Odoo MCP plan:\n${JSON.stringify(plan)}\n\nLIVE factual Odoo results:\n${JSON.stringify(results)}`;const{text,model}=await callAnthropic('You output only the JSON object described in the instructions below — no other text.',PRESENT_SYSTEM_PROMPT+'\n\n'+prompt);const schema=parseJsonLoose(text);if(!validateSchema(schema))throw new Error('Claude returned an invalid UI schema.');return res.json({schema,rawText:text,model});}catch(err){return res.status(err.status||502).json({error:err.message,detail:err.detail});}});
...
const PORT=process.env.PORT||3000;app.listen(PORT,()=>console.log(`Qimam AI Sales — LIVE Odoo MCP — listening on :${PORT}`));
