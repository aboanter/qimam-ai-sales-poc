// Insight Renderer V3.3.3 — post-render enhancement for structured insight arrays.
// Supports both legacy `insights` and current `items` payloads.
(function(){
  ...
  function buildInsight(c){
    ...
    for(const item of items){
      ...
      const mark=document.createElement('span');mark.className='qinsight-mark-v332';mark.textContent=iconFor(item.type,item.source);
  ...
  function enhance(schema,host){
    const insights=(Array.isArray(schema?.components)?schema.components:[])
      .filter(c=>c&&c.type==='insight'&&c.id&&sourceItems(c).length);
    ...
    schema.insightRendererVersion='3.3.3';
  }

  window.render=function insightRenderV333(schema,host){const result=upstream(schema,host);enhance(schema,host);return result;};
...
.qinsight-mark-v332{display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:10px;font-size:12px;font-weight:900;background:color-mix(in srgb,var(--q-accent,#0b9da6) 11%,transparent);color:var(--q-accent,#0b9da6)}
